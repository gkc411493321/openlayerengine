...
var ol_interaction_Transform = class olinteractionTransform extends ol_interaction_Pointer {
  ...
  */
  ...
  // 只用于控件显示，不参与交互计算
    ...
    extWrap[0] += wrapOffset; extWrap[2] += wrapOffset
    var coordsWrap = coords ? coords.map(function(c) { return [c[0] + wrapOffset, c[1]] }) : coords
    if (center === true) {
      if (!this.ispt_) {
        this.overlayLayer_.getSource().addFeature(new ol_Feature({ geometry: new ol_geom_Point([this.center_[0] + wrapOffset, this.center_[1]]), handle: 'rotate0' }))
      ...
      if (this.ispt_) {
        ...
        if (p) {
          ...
          ext2 = ol_extent_boundingExtent([
            map.getCoordinateFromPixel([p[0] - dx, p[1] - dy]),
            map.getCoordinateFromPixel([p[0] + dx, p[1] + dy])
          ])
        }
      }
