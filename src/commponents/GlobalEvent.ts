...
export default class GlobalEvent {
   ...
   * 全局的鼠标左键弹起事件
   */
  private globalMouseLeftUpEvent?: { callback: GlobalMouseEventCallback };
  /**
   * 模块下鼠标左键按下监听器处理方法
   * @param event 鼠标事件
   */
 ...
 */
  ...
  private globalMouseUp(event: MouseEvent): void {
    const coordinate = this.map.getEventCoordinate(event);
    this.globalMouseLeftUpEvent?.callback.call(this, { event, position: toLonLat(coordinate) });
  }
   ...
   * @param callback 回调函数，详见{@link GlobalMouseEventCallback}
   */
  addMouseLeftUpEventByGlobal(callback: GlobalMouseEventCallback): void {
